...
const emit = defineEmits(['update:modelValue', 'setRef', "change"]);
...
const inputElClass = computed(() => {
  const base = [
    ...
    computedType.value === 'textarea' ? 'h-36' : 'h-12',
...
// const computedMultiSelectValue = computed(() => (props.multiSelectValue));
...
<template>
  <div class="relative">
    <!--<Multiselect v-if="computedType === 'select'" v-bind="computedMultiSelectValue" v-model="computedValue" />-->

    <select v-if="computedType === 'select'" :id="id" v-model="computedValue" :name="name" :class="inputElClass"
      :placeholder="placeholder" @change="onChange($event.target.value)" :disabled="disabled">
