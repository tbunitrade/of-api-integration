...
watch(
  ...
  (v: any) => {
    ...
    const curr = Array.isArray(props.audienceIncludeIds) ? (props.audienceIncludeIds as any).map(String) : [];
...
watch(
  ...
  (v: any) => {
    const next = Array.isArray(v) ? v.map(String) : [];
    const curr = Array.isArray(props.audienceExcludeIds) ? (props.audienceExcludeIds as any).map(String) : [];
    if (sameArr(next, curr)) return;
    emit('update:audienceExcludeIds', next);
  },
  { deep: true }
);
